...
import unsw.loopmania.Goals.AndGoal;
import unsw.loopmania.Goals.OrGoal;
...
public abstract class LoopManiaWorldLoader {
    ...
    public LoopManiaWorld load() {
        ...
        Goal goal = loadGoal(world, jsonGoal);
        world.setGoal(goal);

        // load special items
        JSONArray items = json.getJSONArray("rare_items");
        List<String> rareitem = new ArrayList<>();
        
        for (int i = 0; i < items.length(); i++) {
            rareitem.add(items.getString(i));
        }

        world.rareItems(rareitem);
     ...
     * @return
     */
    private Goal loadGoal(LoopManiaWorld world, JSONObject json) {
        ...
        switch (goal_type) {
            ...
            case "OR":
                ...
                break;
            case "experience":
                goal = new ExperienceGoal(world);
                quantity = json.getInt("quantity");
                goal.setQuantity(quantity);
                break;
            case "gold":
                goal = new GoldGoal(world);
                quantity = json.getInt("quantity");
                goal.setQuantity(quantity);
                break;
            case "cycles":
                goal = new CycleGoal(world);
                quantity = json.getInt("quantity");
                goal.setQuantity(quantity);
                break;
            case "bosses":
        ...
        if (goal.hasSubGoals()) {
            ...
            for (int i = 0; i < subGoal.length(); i++) {
                Goal newSubGoal = loadGoal(world, subGoal.getJSONObject(i));
                goal.addSubGoal(newSubGoal);
            } 
        }   
        return goal;
